  ...
  describe Committed do
    ...
    describe 'get_issue_urls' do
      ...
      it 'fails if issue_pattern is not a String or Regexp' do
        expect{ Committed.get_issue_urls(nil, nil, nil) }.to raise_error TypeError
      end

      it 'fails if url_pattern is not a String' do
        expect{ Committed.get_issue_urls(issue_pattern, nil, nil) }.to raise_error TypeError
      end

      it 'fails if message is not a String' do
        expect{ Committed.get_issue_urls(issue_pattern, url_pattern, nil) }.to raise_error TypeError
      end

      it 'returns empty array if there are no issues' do
        expect(Committed.get_issue_urls(issue_pattern, url_pattern, issueless_message)).to match_array []
      end

      it 'returns array with one match if there is one issue' do
        expect(Committed.get_issue_urls(issue_pattern, url_pattern, one_issue_message)).to match_array one_issue
      end

      it 'returns array with two matches if there are two issues' do
        expect(Committed.get_issue_urls(issue_pattern, url_pattern, two_issues_message)).to match_array two_issues
      end

      it 'returns array with two matches if there are two adjoining issues' do
        expect(Committed.get_issue_urls(issue_pattern, url_pattern, two_adjoining_issues_message)).to match_array two_issues
      end

      it 'returns array with two matches if there are two issues over two lines' do
        expect(Committed.get_issue_urls(issue_pattern, url_pattern, two_issues_over_two_lines_message)).to match_array two_issues
      end
    end
